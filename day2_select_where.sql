...
--   ORNEK4: id’si 124 olan öğrencilerin tüm bilgilerini listele
...
/*======================= SELECT - BETWEEN ==================================*/ 
    ...
    -- not: BETWEEN iki mantiksal ifade ile tanimlayabilecğimiz durumlari tek komutla yazabilme imkani verir. yazdğmz 2 snr da aralğa dahildir. 
...
-- Bu örnekte sınav1 veya sınav2 alanı 50'den kucuk kayıtlar listelenecektir.
*/

select * from personel where maas >=7000 and maas<11000;
select * from personel where maas >6000 or maas>13000;
...
/* -----------------------------------------------------------------------------
  ...
  listeleyiniz. 
 -----------------------------------------------------------------------------*/  
 select * from personel where isim between "Mehmet Yilmaz" and "Veli Han" order by isim;
...
/* ========================================================================== */
    ...
    INSERT INTO kelimeler VALUES (1015, 'hoooooooooot', 5);
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
    ...
    -- h ile başlayıp t ile biten 4 harfli kelimeleri (h ile t küçük harfli olanlari) listeleyeniz
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
	select * from kelimeler where REGEXP_like (kelime, 'h[a|i]t');-- ilk yol
    select * from kelimeler where REGEXP_like (kelime, 'h[ai]t'); -- ikinci yol
/* -----------------------------------------------------------------------------
  ORNEK28: İçinde m veya i veya e olan kelimelerin tüm bilgilerini listeleyiniz.
 -----------------------------------------------------------------------------*/     
   select * from kelimeler where regexp_like (kelime, '[mie]'); -- ilk yol
   select * from kelimeler where regexp_like (kelime, 'm|i|e'); -- ikinci yol
/* -----------------------------------------------------------------------------
  ORNEK29: a veya s ile başlayan kelimelerin tüm bilgilerini listeleyiniz.
-----------------------------------------------------------------------------*/ 
	select * from kelimeler where regexp_like (kelime, '^a|^s');-- ilk yol
	select * from kelimeler where regexp_like (kelime, '^[as]');-- ikinci yol 	

/* -----------------------------------------------------------------------------
  ORNEK30: içerisinde en az 2 adet oo barıdıran kelimelerin tüm bilgilerini 
  listeleyiniz.
-----------------------------------------------------------------------------*/ 
	select * from kelimeler where regexp_like (kelime, '[%oo%%][%%oo%%]');
